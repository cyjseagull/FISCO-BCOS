...
void JsonRpcImpl_2_0::loadAndHandleTransaction(std::string const& _groupID,
    std::string cosnt& _nodeName, std::string const& _txsFilePath, int64_t _qps, RespFunc _respFunc)
{
    auto nodeService = getNodeService(_groupID, _nodeName, "loadAndHandleTransaction");
    Error::Ptr error = nullptr;
    Json::Value response;
    std::ifstream in(_txsFilePath.c_str());
    if (!in.is_open())
    {
        std::stringstream ss;
    ...
    std::cout << "load txsData from " << _txsFilePath << std::endl;
    ...
    bcos::protocol::TxSubmitCallback callback =
        [startT, self, totalTxsCount](
            ...
            (rpc->m_receivedTxs)++;
            if (m_receivedTxs == totalTxsCount)
            {
            ...
            if (_error)
            {
                (m_errorTxs)++;
                RPC_IMPL_LOG(WARNING)
                    << LOG_DESC("submitTransaction failed") << LOG_LV("error", _error->errorCode())
    ...
    try
    {
        for (int i = 0; i < txsData.size(); i++)
        {
            auto binTxData = txsData.at(i);
            // deserialize the tx
            auto tx = txFactory->createTransaction(ref(*binTxData), false);
            // submit the tx
            nodeService->txpool()->asyncSubmit(ref(*binTxData), callback);
