...
//根据gpk_str 恢复gpk系统参数
int BBSKey::load_gpk(
    bbs_group_public_key_ptr p_gpk, const string& gpk_str, bbs_sys_param_ptr p_sys_param)
{
    try
    {
        //恢复gpk
        ...
        trans_string_to_element(
            p_gpk->pr_g1_g2_inv, pub_loader.get_value<string>("pr_g1_g2_inv").c_str());
    }
    catch (exception& err)
    {
...
int BBSKey::store_gpk(std::string& gpk_info, bbs_group_public_key_ptr p_gpk)
{
    try
    {
...
//保存私钥信息: v1:私钥不加密
// TODO: 私钥加密存储
int BBSKey::store_gmsk(std::string& gmsk_info, bbs_manager_private_key_ptr p_gmsk)
{
    try
    {
        ...
        LOG(DEBUG) << "PRIVATE KEY FOR GROUP MANAGER: (" << (gmsk["xi1"]) << "," << (gmsk["xi2"])
                   << ")";
...
//加载私钥信息
// TODO: 私钥加密存储 && 解密load
int BBSKey::load_gmsk(
    bbs_manager_private_key_ptr p_gmsk, const std::string& gmsk_info, bbs_sys_param_ptr p_sys_param)
{
    try
    {
...
//###群成员私钥信息管理
// TODO: 私钥加密管理
int BBSKey::store_gsk(std::string& gsk_info, bbs_group_private_key_ptr gsk)
{
    try
    {
        ...
        LOG(DEBUG) << "GSK INFO: (" << (gsk_map["A"]) << "," << (gsk_map["x"]) << ")";
        // gsk信息写入gsk_info
...
// TODO: 加密后私钥解密
int BBSKey::load_gsk(
    bbs_group_private_key_ptr gsk, const std::string& gsk_info, bbs_sys_param_ptr p_sys_param)
{
    try
    {
...

{
    try
    {
        ...
        trans_string_to_element(
            sig_struct.rdelta1, sig_loader.get_value<string>("rdelta1").c_str());
        LOG(DEBUG) << "LOAD rdelta2";
        trans_string_to_element(
            sig_struct.rdelta2, sig_loader.get_value<string>("rdelta2").c_str());
    }
    catch (exception& err)
    {
