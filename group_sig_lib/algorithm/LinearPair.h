...
public:
    ...
    virtual void set_inited() { pbc_param_inited = true; }
    //加载pbc param信息
    virtual pbc_param_ptr load_pbc_param(std::string& pbc_param_str, int& ret_code)
    {
    ...
    virtual pbc_param_ptr get_pbc_param() { return param_ptr; }
...
public:
    ...
    ALinearPair(shared_ptr<ConfigParserInterface<JsonConfigParser>>& config,
      ...
      : LinearPair(config), _q_key(q_key), _r_key(r_key)
    {
        q_bits = DEFAULT_QLEN;
        r_bits = DEFAULT_RLEN;
        //从配置中加载q和r信息
...
public:
    ...
    string _order_key;
    AOneLinearPair(
        shared_ptr<ConfigParserInterface<JsonConfigParser>>& config, string order_key = "order")
      : LinearPair(config), _order_key(order_key)
...
public:
    ...
    ELinearPair(shared_ptr<ConfigParserInterface<JsonConfigParser>>& config,
        string r_key = "r_bits_len", string q_key = "q_bits_len")
      : LinearPair(config), _r_key(r_key), _q_key(q_key)
    {
...
public:
    ...
    string _bits_key;
    FLinearPair(
        shared_ptr<ConfigParserInterface<JsonConfigParser>>& config, string bits_key = "bit_len")
      : LinearPair(config), _bits_key(bits_key)
    {
...
public:
    ...
    ILinearPair(shared_ptr<ConfigParserInterface<JsonConfigParser>>& config,
        std::string group_key = "group_size")
      : LinearPair(config), _group_key(group_key)
    {
