...
{
    size_t count = m_groupPBFTReqCache->getGlobalViewChangeSize(m_toView);
    if (count >= (size_t)(minValidNodes() - 1))
    {
        broadcastSuperViewChangeReq(1);
        checkGlobalSuperViewChange();
    }
    else
    {
        count = m_groupPBFTReqCache->getViewChangeSize(m_toView);
        if (count >= (size_t)(minValidNodes() - 1))
        {
...
virtual void checkGlobalSuperViewChange() {}
...
{
    {
        ...
        if (!locatedInConsensusZone() && m_timeManager.isTimeout())
        {
...
std::shared_ptr<PBFTMsg> GroupPBFTEngine::handleMsg(std::string& key, PBFTMsgPacket const& pbftMsg)
{
    ...
    case GroupPBFTPacketType::ViewChangePacket:
    {
        std::shared_ptr<GroupViewChangeReq> globalViewChange =
            std::make_shared<GroupViewChangeReq>();
        succ = handleViewChangeMsg(globalViewChange, pbftMsg);
        key = globalViewChange->uniqueKey();
        pbftPacket = globalViewChange;
        break;
    }
    default:
    {
