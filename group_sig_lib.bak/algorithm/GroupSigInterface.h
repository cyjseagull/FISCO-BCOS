...
public:
    GroupSigInterface(const std::string& config_path, std::string dir = "data")
      : gpk_info_dir(dir), linear_config_path(config_path), m_lock(Locker())
    {
        if ("" == dir)
            gpk_info_dir = "data";
    ...
    std::string get_gpk_dir() { return gpk_info_dir; }
