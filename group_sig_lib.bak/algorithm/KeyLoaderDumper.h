...
public:
    ...
    static int store_gpk(std::string& gpk_info, bbs_group_public_key_ptr p_gpk);


    //保存私钥信息: v1:私钥不加密
    // TODO: 私钥加密存储
    static int store_gmsk(std::string& gmsk_info, bbs_manager_private_key_ptr p_gmsk);

    //加载私钥信息
    // TODO: 私钥加密存储 && 解密load
    static int load_gmsk(bbs_manager_private_key_ptr p_gmsk, const std::string& gmsk_info,
        bbs_sys_param_ptr p_sys_param);

    //###群成员私钥信息管理
    // TODO: 私钥加密管理
    static int store_gsk(std::string& gsk_info, bbs_group_private_key_ptr gsk);
