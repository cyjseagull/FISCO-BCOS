...
public:
    ...
    JsonConfigParser(string const& json_str, JsonConfigType type = JsonFile) : input_type(type)
    {
        if (JsonString == type)
        {
            try
            ...
            {
                LOG(ERROR) << "LOAD JSON STR:" << json_str
                           << " FAILED, error_msg:" << error_msg.what();
            }
        }
        // default is JsonFile
        else
        {
            ...
            {
                LOG(ERROR) << "LOAD JSON FROM FILE " << json_str
                           << " FAILED, error_msg:" << error_msg.what();
            }
        }
    }
    ...
    template <typename T>
    static void convert_to_json_str(std::string& json_str, std::map<string, T> value_map)
    {
        ptree root_json;
