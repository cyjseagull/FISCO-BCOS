...
static inline int string_cmp_ignore_case(const std::string& lhs, const std::string& rhs)
{
#if defined(_WIN32)
...
static inline void trans_to_linear_type(LinearPairType& type, const std::string& type_str)
{
 ...
 * @ret: pbc param字符串
 */
static inline string dump_pbc_param(const string& group_name, string& dir, pbc_param_ptr pbc_param)
{
    assert(pbc_param);
...
static inline string trans_to_string(element_ptr big_num, int len = -1)
{
    ...
    {
        LOG(ERROR) << "trans big_num to string failed, error msg:" << err.what();
        return err.what();
    }
}
...
static inline void trans_string_to_element(element_ptr p_big_num, const string& e_str)
{
    try
    {
        assert(p_big_num);
    ...
    {
        LOG(ERROR) << "trans e_str " << e_str << " to big num failed, error_msg:" << err.what();
    }
}
...
static inline void get_substring(std::string& result, const std::string& delim, unsigned int index)
{
    ...
    LOG(DEBUG) << "get_substring, index:" << index << " result:" << result;
}
